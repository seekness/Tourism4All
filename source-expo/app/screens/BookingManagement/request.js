...
import {bookingRequestSelect} from '@selectors';
import styles from './styles';

export default memo(BookingRequest);

function BookingRequest(props) {
  const {t} = useTranslation();
  const {theme} = useContext(Application);
  const booking = useSelector(bookingRequestSelect);
  ...
  useEffect(() => {
    dispatch(bookingActions.onLoadRequest({}));
  }, [dispatch]);
  ...
  const onLoad = ({refresh, params}) => {
    ...
    dispatch(
      bookingActions.onLoadRequest({sort, status, keyword, ...params}, () => {
        setRefreshing(false);
      }),
    );
  };
  ...
  const onMore = () => {
    if (booking?.pagination?.allowMore) {
      dispatch(bookingActions.onLoadMoreRequest({sort, status, keyword}));
    }
  };
